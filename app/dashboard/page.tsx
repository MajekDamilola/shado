        ...
        {!connected ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button onClick={() => setActiveTab(activeTab === "receive" ? null : "receive")} className="py-4 rounded-xl font-semibold text-sm transition" style={{
            ...
            {activeTab === "send" && (
              <div className="rounded-2xl p-6 flex flex-col gap-4" style={{background: "var(--bg-card)", border: "1px solid rgba(128,128,128,0.1)"}}>
                ...
                <div className="flex items-center rounded-xl px-4 py-3 gap-2 border" style={{background: "var(--bg-main)", borderColor: "rgba(128,128,128,0.15)"}}>
                  <span style={{color: "#FF6B00"}}>@</span>
                  <input type="text" placeholder="recipient handle" value={sendHandle}
                    onChange={(e) => setSendHandle(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ""))}
                    className="bg-transparent outline-none flex-1 placeholder:text-zinc-600 text-sm" style={{color: "var(--text-main)"}}
                  />
                </div>
                <div className="flex items-center rounded-xl px-4 py-3 gap-2 border" style={{background: "var(--bg-main)", borderColor: "rgba(128,128,128,0.15)"}}>
                ...
                <button disabled={!sendHandle || !sendAmount} className="w-full py-3.5 rounded-xl text-sm font-semibold transition" style={{
                ...
                }}>Send to @{sendHandle || "handle"} →</button>
              </div>
            )}
