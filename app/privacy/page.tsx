  ...
  return (
    <main className="min-h-screen overflow-x-hidden" style={{background: "var(--bg-main)", color: "var(--text-main)"}}>
      ...
      <section className="pt-40 pb-32 px-8 max-w-4xl mx-auto">
        ...
        <h1 style={{fontSize: "56px", fontWeight: "800", letterSpacing: "-2px", marginBottom: "16px", color: "var(--text-main)"}}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-16">
          {[
            ...
            <div key={item.title} className="rounded-2xl p-6" style={{background: "var(--bg-card)", border: "1px solid rgba(128,128,128,0.1)"}}>
              <div className="flex items-center gap-3 mb-3">
                <div className="w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0" style={{background: "rgba(255,107,0,0.1)", border: "1px solid rgba(255,107,0,0.15)"}}>
                  <span style={{color: "#FF6B00", fontSize: "10px"}}>✓</span>
                </div>
                <h3 style={{fontSize: "15px", fontWeight: "700", color: "var(--text-main)"}}>{item.title}</h3>
              </div>
        ...
        <div className="rounded-3xl p-10 relative overflow-hidden" style={{background: "var(--bg-card)", border: "1px solid rgba(255,107,0,0.1)"}}>
          <div className="absolute top-0 right-0 pointer-events-none" style={{width: "300px", height: "300px", background: "radial-gradient(ellipse at top right, rgba(255,107,0,0.06) 0%, transparent 70%)"}}/>
          <p style={{color: "#FF6B00", fontSize: "11px", letterSpacing: "3px", marginBottom: "16px"}}>THE TECH</p>
          <h2 style={{fontSize: "28px", fontWeight: "800", letterSpacing: "-1px", marginBottom: "16px", color: "var(--text-main)"}}>Powered by Miden ZK rollup</h2>
          <p style={{color: "var(--text-muted)", fontSize: "15px", lineHeight: "1.8", maxWidth: "560px"}}>
            Miden uses client-side proving — your device generates ZK proofs locally before anything is sent to the network. The rollup node only ever sees cryptographic commitments, never your actual data.
          </p>
        </div>
      </section>
