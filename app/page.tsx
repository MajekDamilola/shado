    ...
    <main className="min-h-screen overflow-x-hidden" style={{background: "var(--bg-main)", color: "var(--text-main)"}}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-8 py-4" style={{
        ...
        <div className="flex items-center gap-3 relative">
          ...
          >
            <span className="block w-4 h-px" style={{background: "var(--text-main)", transform: menuOpen ? "rotate(45deg) translate(2px, 2px)" : "none", transition: "all 0.2s"}}/>
            <span className="block h-px" style={{background: "var(--text-main)", width: menuOpen ? "16px" : "12px", opacity: menuOpen ? 0 : 1, transition: "all 0.2s"}}/>
            <span className="block h-px" style={{background: "var(--text-main)", width: menuOpen ? "16px" : "8px", transform: menuOpen ? "rotate(-45deg) translate(2px, -2px)" : "none", transition: "all 0.2s"}}/>
          </button>

          {menuOpen && (
            <div className="absolute top-full right-0 w-56 py-3 px-3 flex flex-col gap-1 rounded-2xl mt-2" style={{
              background: "var(--nav-bg)",
              backdropFilter: "blur(20px)",
              border: "1px solid rgba(128,128,128,0.15)",
              boxShadow: "0 20px 40px rgba(0,0,0,0.3)"
            }}>
              <Link href="/how" onClick={() => setMenuOpen(false)} className="py-2.5 px-3 text-sm font-medium rounded-xl" style={{color: "var(--text-muted)"}}>How it works</Link>
              <Link href="/privacy" onClick={() => setMenuOpen(false)} className="py-2.5 px-3 text-sm font-medium rounded-xl" style={{color: "var(--text-muted)"}}>Privacy</Link>
              <Link href="/dashboard" onClick={() => setMenuOpen(false)} className="py-2.5 px-3 text-sm font-medium rounded-xl" style={{color: "var(--text-muted)"}}>Dashboard</Link>
              <div className="my-1" style={{height: "1px", background: "rgba(128,128,128,0.15)"}}/>
              <Link href="/register" onClick={() => setMenuOpen(false)} className="py-2.5 px-3 text-center text-sm font-semibold rounded-xl" style={{background: "linear-gradient(135deg, #FF6B00, #FF8C00)", color: "#fff"}}>
                Get started →
              </Link>
            </div>
          )}
      ...
      <section className="relative flex flex-col items-center justify-center text-center px-6 pt-40 pb-32">
        <div className="inline-flex items-center gap-2 text-xs font-medium px-4 py-2 rounded-full mb-10" style={{
          ...
          Built on Miden by @just_marhk
        </div>

        <h1 style={{fontSize: "clamp(48px, 8vw, 96px)", fontWeight: "800", letterSpacing: "-3px", lineHeight: "1", marginBottom: "28px", color: "var(--text-main)"}}>
          Send crypto.<br/>
        ...
        <p className="max-w-xl mx-auto mb-12" style={{color: "var(--text-muted)", fontSize: "18px", lineHeight: "1.7"}}>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 mb-24">
          <Link href="/register" className="px-8 py-4 rounded-xl font-semibold text-sm" style={{
            background: "linear-gradient(135deg, #FF6B00, #FF8C00)", color: "#fff",
            boxShadow: "0 0 30px rgba(255,107,0,0.3)", minWidth: "200px", textAlign: "center"
          }}>Claim your handle →</Link>
          <Link href="/how" className="px-8 py-4 rounded-xl text-sm font-medium" style={{
            background: "rgba(128,128,128,0.08)", border: "1px solid rgba(128,128,128,0.15)",
            color: "var(--text-muted)", minWidth: "200px", textAlign: "center"
          }}>See how it works</Link>
        </div>

        <div className="w-full max-w-sm mx-auto rounded-2xl p-6" style={{
          background: "var(--bg-card)", border: "1px solid rgba(128,128,128,0.12)",
          boxShadow: "0 40px 80px rgba(0,0,0,0.3)"
        }}>
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-green-400" style={{boxShadow: "0 0 6px #4ade80"}}/>
              <span style={{color: "var(--text-muted)", fontSize: "12px"}}>@tunde</span>
            </div>
            <span style={{color: "var(--text-muted)", fontSize: "11px", letterSpacing: "2px", opacity: 0.4}}>PRIVATE</span>
          </div>
          <div className="mb-6">
            <p style={{color: "var(--text-muted)", fontSize: "11px", letterSpacing: "2px", marginBottom: "6px", opacity: 0.5}}>BALANCE</p>
            <p style={{fontSize: "42px", fontWeight: "800", letterSpacing: "-2px", color: "var(--text-main)"}}>$0.00</p>
            <p style={{color: "var(--text-muted)", fontSize: "11px", marginTop: "4px", opacity: 0.4}}>Only you can see this</p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="py-3 rounded-xl text-center text-sm font-medium" style={{background: "rgba(128,128,128,0.08)", border: "1px solid rgba(128,128,128,0.12)", color: "var(--text-muted)"}}>↓ Receive</div>
          </div>
        </div>
      </section>

      <div className="border-y py-6" style={{borderColor: "rgba(128,128,128,0.1)"}}>
        <div className="max-w-4xl mx-auto grid grid-cols-3 gap-8 px-8 text-center">
          {[
            ...
            <div key={s.label}>
              ...
              <p style={{fontSize: "12px", color: "var(--text-muted)", marginTop: "4px", opacity: 0.6}}>{s.label}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <section className="py-32 px-8 max-w-5xl mx-auto">
        <div className="text-center mb-20">
          ...
          <h2 style={{fontSize: "42px", fontWeight: "800", letterSpacing: "-2px", color: "var(--text-main)"}}>Private by design</h2>
          <p style={{color: "var(--text-muted)", fontSize: "16px", marginTop: "12px"}}>Every layer built for zero exposure</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={f.title} className="rounded-2xl p-7" style={{background: "var(--bg-card)", border: "1px solid rgba(128,128,128,0.1)"}}>
              ...
              <h3 className="font-bold mb-3" style={{fontSize: "17px", letterSpacing: "-0.5px", color: "var(--text-main)"}}>{f.title}</h3>
      ...
      <section className="py-32 px-8 max-w-5xl mx-auto">
        <div className="rounded-3xl p-12 relative overflow-hidden" style={{background: "var(--bg-card)", border: "1px solid rgba(255,107,0,0.1)"}}>
          ...
          <div className="max-w-xl relative">
            ...
            <h2 style={{fontSize: "36px", fontWeight: "800", letterSpacing: "-1.5px", marginBottom: "16px", color: "var(--text-main)"}}>What no one can ever see</h2>
            <p style={{color: "var(--text-muted)", fontSize: "15px", lineHeight: "1.7", marginBottom: "32px"}}>Shado is built on Miden's ZK rollup. Client-side proving means your data never leaves your device.</p>
      ...
      <section className="text-center px-6 py-32">
        ...
        <h2 style={{fontSize: "56px", fontWeight: "800", letterSpacing: "-2px", marginBottom: "16px", color: "var(--text-main)"}}>Ready to go dark?</h2>
        <p style={{color: "var(--text-muted)", fontSize: "16px", marginBottom: "40px"}}>Claim your handle. Start sending privately.</p>
        <Link href="/register" className="inline-block px-12 py-5 rounded-xl font-bold" style={{
          background: "linear-gradient(135deg, #FF6B00, #FF8C00)", color: "#fff",
          boxShadow: "0 0 40px rgba(255,107,0,0.35)", fontSize: "15px"
        }}>Claim @handle →</Link>
      </section>
